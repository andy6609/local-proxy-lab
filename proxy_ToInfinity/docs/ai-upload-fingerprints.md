...
- ⚠️ **이슈3 (신규) — 동시 쓰기 레이스로 저장 파일 손상.**
  ...
  - ✅ **수정됨(2026-07-21):** 저장 파일명에 원자적 시퀀스 접두어(`<seq>_<이름>`) 적용(`std::atomic`). 검증: 같은 파일명 두 **동시** 업로드가 `1_race.txt`/`2_race.txt`로 **각각 원본과 바이트 일치**하게 저장됨(겹침 없음). → claude.ai의 두 엔드포인트 body도 이제 각각 온전히 저장돼, 어느 쪽이 DRM본/평문인지도 분리 확인 가능.
