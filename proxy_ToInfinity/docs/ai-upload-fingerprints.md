...
### ⭐⭐ 결정적 관측 — DRM은 "프로세스/지속(persistent) 기반" + 검증 함정 (2026-07-21)
예외폴더에 둔 유니크 파일 `EXCTEST_plaintext.docx`(git bash로 읽으면 **PK 평문 13699**)를 크롬으로 claude.ai에 올렸더니 → **역시 19856 DRMONE 암호문**으로 나갔다.
- **해석:** 검증에 쓴 **git bash는 Fasoo 허가(authorized) 프로세스라 복호화된 평문(13699)을 본다.** 하지만 **업로더인 크롬(비허가)은 같은 파일을 암호문(19856)으로 읽어 올린다.** (게다가 EXCTEST는 DRM 문서를 `cp`한 파생본 → Fasoo **지속보호**로 암호문 유지. 07-14에 예외폴더 파일이 평문으로 나갔던 건 그때가 **진짜 새로 만든 비-DRM 파일**이었기 때문.)
- ⚠️ **검증 교훈:** "이 파일이 평문인가"를 **git bash(허가 프로세스)로 판단하면 안 된다.** 프록시가 잡는 건 **업로드 앱(크롬)이 실제로 보내는 바이트**다. → 앞으로 평문/DRM 판정은 **프록시 캡처(크롬이 보낸 것) 기준**으로.
- **정리(DLP 최종):** 프록시가 얻는 내용 = **업로드 앱이 읽도록 허가된 내용.**
  - 비-DRM 파일 → **평문(내용까지)** — rootCA.crt 업로드로 바이트 일치 확인(curl 경로).
  - DRM 파일 → **암호문(메타데이터만)** — 파일명·크기·타입·엔드포인트는 안정적으로 획득, 본문은 못 읽음(읽을 때마다 암호문도 달라짐).
