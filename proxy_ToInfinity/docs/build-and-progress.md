...
- ⚠️ **갱신(2026-07-21) — 위 "평문·바이트 일치" 결과는 app 의존적:** 이 13699B 평문 결과는 **curl 업로드**(Fasoo 허가 프로세스)로 얻은 것. **같은 파일이라도 크롬으로 올리면 DRM 암호문(19856B `DRMONE`)** 으로 나간다 — Fasoo DRM이 **프로세스 기반**이라 읽는 앱이 허가됐는지에 따라 평문/암호문이 갈린다. 즉 **"예외폴더=(모든 앱에) 평문"은 참이 아니다.** → 평문/DRM 판정은 **프록시가 잡은 것(=업로드 앱이 실제 보낸 바이트)** 기준으로. 상세·교훈: `ai-upload-fingerprints.md` "결정적 관측 — DRM은 프로세스/지속 기반".
