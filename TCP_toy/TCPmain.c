...
#include <WS2tcpip.h>   // 그 이후 추가된 TCP/IP 관련 확장 기능들 — inet_pton/inet_ntop (문자열 ↔ IP 변환), getaddrinfo(도메인 이름 → IP 변환, 동윤님이 다음 단계에 쓸 함수!), IPv6 관련 구조체 등
...
//                        Go가 내부적으로 sin_family 설정해줌*/
    ...
    왜냐하면 컴퓨터는 4바이트로 처리 하기 때문!! 223.130.192.xxx 이면 [223][130][192] 이런식으로 처리 함 
